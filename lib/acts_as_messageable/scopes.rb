  ...
  module Scopes
    ...
    module ClassMethods
      ...
      def initialize_scopes(search_scope)
        # Track initialized search scopes to avoid warnings in development mode
        @initialized_search_scopes ||= []
        
        # Skip if base scopes are already defined and this specific search scope is initialized
        if respond_to?(:are_from) && @initialized_search_scopes.include?(search_scope)
          return
        end
        
        # Mark this search scope as initialized
        @initialized_search_scopes << search_scope unless @initialized_search_scopes.include?(search_scope)

        # Define base scopes only if not already defined
        unless respond_to?(:are_from)
        ...
        # Define the search scope (can vary per model using the same message class)
