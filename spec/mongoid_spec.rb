...
describe 'ActsAsMessageable with Mongoid' do
  ...
  describe 'send messages with bang' do
    ...
    it 'should return message object' do
      expect(@alice.send_message!(@bob, body: 'body', topic: 'topic')).to be_kind_of(ActsAsMessageable::Mongoid::Message)
    end
  end

  describe 'inheritance models' do
    it 'men send message to alice' do
      send_mongoid_message(@men, @alice)
    ...
    it 'men send message and receive from alice' do
      send_mongoid_message(@men, @alice)
      send_mongoid_message(@alice, @men)
  ...
  it 'messages should return in right order :created_at' do
    @message = send_mongoid_message(@bob, @alice, 'Example', 'Example Body')
