...
describe 'Scope Redefinition' do
  it 'does not produce warnings when acts_as_messageable is called multiple times' do
    ...
    begin
      ...
      scope_warnings = warnings.select { |w| w.include?('Overwriting existing method') || w.include?('Creating scope') }
      
      expect(scope_warnings).to be_empty, "Expected no scope redefinition warnings, but got: #{scope_warnings.join("\n")}"
    ensure
  ...
  it 'keeps scopes functional after multiple initialization calls' do
    ...
    message1 = send_message(@bob, @alice, 'Topic 1', 'Body 1')
